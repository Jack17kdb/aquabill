...
export const getInvoices = async (req, res) => {
  try {
    ...
    const filter = { ownerId: req.adminId };
    if (month) {
...
export const getInvoiceById = async (req, res) => {
  try {
    const invoice = await Invoice.findOne({ _id: req.params.id, ownerId: req.adminId });
...
export const retryFailed = async (req, res) => {
  try {
    ...
    const failed = await Invoice.find({ ownerId: req.adminId, whatsappStatus: 'failed' });
    ...
    await Invoice.updateMany(
      { ownerId: req.adminId, whatsappStatus: 'failed' },
