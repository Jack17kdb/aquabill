...
export const createProperty = async (req, res) => {
  try {
    ...
    const existing = await PropertyUnit.findOne({ ownerId: req.user._id, houseNumber: data.houseNumber });
    if (existing) {
...
export const getAllProperties = async (req, res) => {
  try {
    const properties = await PropertyUnit.find({ ownerId: req.user._id }).sort({ houseNumber: 1 });
    res.json({ properties });
  } catch (error) {
...
export const getStats = async (req, res) => {
  try {
    const total = await PropertyUnit.countDocuments({ ownerId: req.user._id });
    const vacant = await PropertyUnit.countDocuments({ ownerId: req.user._id, isVacant: true });
    ...
    const waterBilledResult = await Invoice.aggregate([
      { $match: { ownerId: req.user._id, createdAt: { $gte: currentMonth } } },
